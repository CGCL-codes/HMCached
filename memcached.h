...
typedef struct __attribute__ ((__packed__)) _stritem {
    ...
    struct _stritem *next;   // for Multi-Queue
    struct _stritem *prev;   // for Multi-Queue
...
typedef struct __attribute__ ((__packed__)) _slabbed_item_nvm {
    uint32_t                    memory_is_dram:1;
    uint32_t                    it_flags:8;
    uint32_t                    slabs_clsid:8;
    uint32_t                    padding:15;

    struct _slabbed_item_nvm    *next;
    struct _slabbed_item_nvm    *prev;
...
typedef struct __attribute__ ((__packed__)) _slabbed_index_nvm {
    struct _slabbed_index_nvm   *next;
    struct _slabbed_index_nvm   *prev;
    uint32_t                    padding;
} slabbed_index_nvm;
...
inline uint64_t ITEM_get_cas(void *it);
inline void ITEM_set_cas(void *it, uint64_t v);
inline char * ITEM_key(void *it);
inline char * ITEM_suffix(void *it);
inline char * ITEM_data(void *it);
inline size_t ITEM_ntotal(void *it);
inline uint8_t ITEM_clsid(void *it);
inline uint8_t ITEM_lruid(void *it);
...
inline uint64_t ITEM_get_cas(void *it) {
    assert(it != NULL);
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return ((it_dram->it_flags & ITEM_CAS) ? it_dram->data->cas : (uint64_t)0);
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return ((it_nvm->it_flags & ITEM_CAS) ? it_nvm->data->cas : (uint64_t)0);
    }
}
...
inline void ITEM_set_cas(void *it, uint64_t v) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        if (it_dram->it_flags & ITEM_CAS)
            it_dram->data->cas = v;
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        if (it_nvm->it_flags & ITEM_CAS)
            it_nvm->data->cas = v;
    }
}
...
inline char * ITEM_key(void *it) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return (((char*)&(it_dram->data)) + ((it_dram->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return (((char*)&(it_nvm->data)) + ((it_nvm->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    }
}
...
inline char * ITEM_suffix(void *it) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return ((char*)&(it_dram->data) + 1 + ((it_dram->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return ((char*)&(it_nvm->data) + 1 + ((it_nvm->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    }
}
...
inline char * ITEM_data(void *it) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return ((char*)&(it_dram->data) + it_dram->nkey + 1 + it_dram->nsuffix
            + ((it_dram->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return ((char*)&(it_nvm->data) + it_nvm->nkey + 1 + it_nvm->nsuffix
            + ((it_nvm->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0));
    }
}
...
inline size_t ITEM_ntotal(void *it) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return sizeof(struct _stritem) + it_dram->nkey + 1
            + it_dram->nsuffix + it_dram->nbytes
            + ((it_dram->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0);
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return sizeof(struct _stritem_nvm) + it_nvm->nkey + 1
            + it_nvm->nsuffix + it_nvm->nbytes
            + ((it_nvm->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0);
    }
}
...
inline uint8_t ITEM_clsid(void *it) {
    ...
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram) {
        return it_dram->slabs_clsid & ~(3 << 6);
    } else {
        item_nvm *it_nvm = (item_nvm *)it;
        return it_nvm->slabs_clsid & ~(3 << 6);
    }
}
...
#undef X
    ...
    uint64_t lru_hits[POWER_LARGEST_MQ];
    // TODO TODO TODO
};
...
struct stats {
    ...
    uint64_t      lru_maintainer_juggles_nvm;
...
struct settings {
    ...
    uint32_t threshold_adjust_period;
...
extern struct stats stats_nvm;
extern struct stats_state stats_state_nvm;


#define ITEM_LINKED 1   // 是否在链表中
// #define ITEM_CAS 2   // 是否CAS  TODO 修改：在前面定义
...
#define ITEM_ACTIVE 16   // 是否Active，在LRU检查时被置位0，在fetch时被置位1
...
typedef struct _stritem {
    ...
    uint16_t        lru_id;
    uint8_t         mq_num;        // lru_id = mq_num + ITEM_clsid(it) * MQ_COUNT
    bool            has_extra_copy;
    uint8_t         reset;
    uint32_t        counter;
...
typedef struct _stritem_nvm {
    bool                memory_is_dram;
    int                 nbytes;
    uint8_t             nsuffix;
    uint8_t             nkey;

    // struct _stritem_nvm *next;
    // struct _stritem_nvm *prev;
...
typedef struct __attribute__ ((__packed__)) _crawler {
    uint32_t        memory_is_dram:1;
    uint32_t        it_flags:8;
    uint32_t        slabs_clsid:8;
    uint32_t        nsuffix:7;
...
typedef struct __attribute__ ((__packed__)) _decayer {
    uint32_t        memory_is_dram:1;
    uint32_t        it_flags:8;
    uint32_t        slabs_clsid:8;
    uint32_t        nsuffix:7;
...
enum store_item_type do_store_item_nvm(item_nvm *item, int comm, conn *c, const uint32_t hv);
...
enum delta_result_type do_add_delta_nvm(conn *c, const char *key,
                                        const size_t nkey, const bool incr,
                                        const int64_t delta, char *buf,
                                        uint64_t *cas, const uint32_t hv);
...
enum delta_result_type add_delta_nvm(conn *c, const char *key,
                                     const size_t nkey, const int incr,
                                     const int64_t delta, char *buf,
                                     uint64_t *cas);
...
// NVM_ZONE
item *item_alloc_nvm(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes);
item *item_get_nvm(const char *key, const size_t nkey, conn *c, const bool do_update);
item *item_touch_nvm(const char *key, const size_t nkey, uint32_t exptime, conn *c);
int   item_link_nvm(item *it);
void  item_remove_nvm(item *it);
int   item_replace_nvm(item *it, item *new_it, const uint32_t hv);
void  item_unlink_nvm(item *it);

void item_lock_nvm(uint32_t hv);
void *item_trylock_nvm(uint32_t hv);
void item_trylock_unlock_nvm(void *arg);
void item_unlock_nvm(uint32_t hv);


void pause_threads(enum pause_thread_types type);
//#define refcount_incr(it) ++(it->refcount)
//#define refcount_decr(it) --(it->refcount)
//#define refcount_incr_nvm(it) ++(it->index->refcount)
//#define refcount_decr_nvm(it) --(it->index->refcount)
inline uint8_t refcount_incr(void *it);
inline uint8_t refcount_decr(void *it);

inline uint8_t refcount_incr(void *it) {
    assert(it != NULL);
    item *it_dram = (item *)it;
    if (it_dram->memory_is_dram)
        return ++(it_dram->refcount);
    else
        return ++(((item_nvm *)it)->index->refcount);
}
...
// NVM_ZONE
void STATS_LOCK_NVM(void);
void STATS_UNLOCK_NVM(void);
...
// NVM_ZONE
enum store_item_type store_item_nvm(item *item, int comm, conn *c);
...
unsigned get_dram_capacity(void);
...
item *do_item_decayer_q_nvm(item *it);
