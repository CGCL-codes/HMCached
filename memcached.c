...
#ifdef NVM_ZONE
struct stats stats_nvm;
struct stats_state stats_state_nvm;
#endif
...
#ifdef NVM_ZONE
    memset(&stats_nvm, 0, sizeof(struct stats));
    memset(&stats_state_nvm, 0, sizeof(struct stats_state));
    stats_state_nvm.accepting_conns = true;
#endif
...
static void process_bin_get_or_touch(conn *c) {
    // TODO printf("come in process_bin_get_or_touch()\n");
    item *it;
    ...
    /* LEEZW
    if (it && it->memory_is_dram == 1 && it->refcount > IT_REFCOUNT_LIMIT) {
        item_remove(it);
        it = NULL;
    } else if (it && it->memory_is_dram == 0 && ((item_nvm *)it)->meta->refcount > IT_REFCOUNT_LIMIT) {
        item_remove(it);
        it = NULL;
    }
    */
...
static void process_bin_update(conn *c) {
    ...
    size_t ntotal = sizeof(item) + nkey + 1 + vlen + 2;
    unsigned id = slabs_clsid(ntotal);
    ...
    if (dit != NULL || suit_dram(id, counter))
        it = item_alloc(key, nkey, req->message.body.flags,
                        realtime(req->message.body.expiration), vlen+2);
    else
        it = item_alloc_nvm(key, nkey, req->message.body.flags,
                            realtime(req->message.body.expiration), vlen+2);
...
static int _store_item_copy_data_nvm(int comm, item_nvm *old_it, item_nvm *new_it, item_nvm *add_it) {
    if (comm == NREAD_APPEND) {
        if (new_it->it_flags & ITEM_CHUNKED) {
            return -1;
        } else {
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    ...
    item *old_it = assoc_find(key, it->nkey, hv);
    if (old_it != NULL)
    ...
    } else if (!old_it && ( /* comm == NREAD_REPLACE || TODO */
        ...
        if (stored == NOT_STORED && failed_alloc == 0) {
            ...
            if (old_it != NULL) {
                ...
                if (nit) {
                    ...
                    do_item_unlink_nvm(nit, hv);
                } else {
                    assert(it->counter == 0);
                    it->counter = settings.set_incr;
                    while (!lockfree_push(ITEM_clsid(it), 0, it->counter));
                }
                do_item_link(it, hv, true);
            }
...
enum store_item_type do_store_item_nvm(item_nvm *it, int comm, conn *c, const uint32_t hv) {
    char *key = ITEM_key(it);
    // item_nvm *old_it = do_item_get_nvm(key, it->nkey, hv, c, DONT_UPDATE);
    item_nvm *old_it = assoc_find_nvm(key, it->nkey, hv);
    if (old_it != NULL)
        refcount_incr(old_it);

    enum store_item_type stored = NOT_STORED;
    
    item_nvm *new_it = NULL;
    uint32_t flags;
    
    if (old_it != NULL && comm == NREAD_ADD) {
        /* add only adds a nonexistent item, but promote to head of LRU */            
        do_item_update_nvm(old_it);
    } else if (!old_it && ( /*  comm == NREAD_REPLACE || TODO */                    
                comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {               
        /* replace only replaces an existing value; don't store */
        ...
        if(old_it == NULL) {
            ...
            STORAGE_delete(c->thread->storage, old_it);
            item_replace_nvm((item *)old_it, (item *)it, hv);
            stored = STORED;
        } else {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.slab_stats[ITEM_clsid(old_it)].cas_badval++;
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                // LEEZW
                new_it = do_item_alloc_nvm(key, it->nkey, flags, old_it->exptime, it->nbytes + old_it->nbytes - 2 /*  CRLF */);
                
                /* copy data from it and old_it to new_it */
                if (new_it == NULL || _store_item_copy_data_nvm(comm, old_it, new_it, it) == -1) {
                    failed_alloc = 1;
                    stored = NOT_STORED;      
                    // failed data copy, free up.
                    if (new_it != NULL)                    
                        item_remove_nvm((item *)new_it);
                } else {
                    it = new_it;
                }
            }
        ...
        if (stored == NOT_STORED && failed_alloc == 0) {
            ...
            if (old_it != NULL) {
                ...
                while (!lockfree_push(ITEM_clsid(it), old_it->index->counter, it->index->counter));
                item_replace_nvm((item *)old_it, (item *)it, hv);
            } else {
                ...
                if (dit) {
                    ...
                    while (!lockfree_push(ITEM_clsid(it), dit->counter, it->index->counter));
                    do_item_unlink(dit, hv);
                } else {
                    assert(it->index->counter == 0);
                    it->index->counter = settings.set_incr;
                    while (!lockfree_push(ITEM_clsid(it), 0, it->index->counter));
                }
                do_item_link_nvm(it, hv, true);
            } 
    ...
    if (old_it != NULL)
        do_item_remove_nvm(old_it);         /* release our reference */
    if (new_it != NULL)
        do_item_remove_nvm(new_it);
...
static void server_stats(ADD_STAT add_stats, conn *c) {
    ...
    APPEND_STAT("limit_maxbytes_nvm", "%llu", (unsigned long long)settings.maxbytes_nvm);
...
#ifdef NVM_ZONE
    if (settings.lru_maintainer_thread_nvm) {
        APPEND_STAT("lru_maintainer_juggles_nvm", "%llu", (unsigned long long)stats.lru_maintainer_juggles_nvm);
    }
#endif
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("maxbytes_nvm", "%llu", (unsigned long long)settings.maxbytes_nvm);
...
static inline item* limited_get(char *key, size_t nkey, conn *c, uint32_t exptime, bool should_touch) {
    ...
    if (it && it->memory_is_dram == 1 && it->refcount > IT_REFCOUNT_LIMIT) {
        item_remove(it);
        it = NULL;
    } else if (it && it->memory_is_dram == 0 && ((item_nvm *)it)->index->refcount > IT_REFCOUNT_LIMIT) {
        item_remove(it);
        it = NULL;
    }
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    size_t ntotal  = sizeof(item) + nkey + 1 + vlen;
    unsigned id = slabs_clsid(ntotal);
    uint32_t counter = 0;
    uint32_t hv = hash(key, nkey);
    item_lock(hv);
    ...
    if (dit != NULL || suit_dram(id, counter))
        ...
        it = item_alloc_nvm(key, nkey, flags, realtime(exptime), vlen);
    ...
    if (it == 0) {
         ...
         * Unacceptable for SET. Anywhere else too? */
        if (comm == NREAD_SET) {
            printf("LEEZW: not happened in here, process_update_command()\n");
            it = item_get(key, nkey, c, DONT_UPDATE);
            if (it) {
                item_unlink(it);
                STORAGE_delete(c->thread->storage, it);
...
enum delta_result_type do_add_delta_nvm(conn *c, const char *key, const size_t nkey,
                                        const bool incr, const int64_t delta,
                                        char *buf, uint64_t *cas, const uint32_t hv)
{
    printf("LEEZW: not happened in here, do_add_delta_nvm()\n");
    char *ptr;
    uint64_t value;    
    int res;
    item_nvm *it;
    it = do_item_get_nvm(key, nkey, hv, c, DONT_UPDATE);
    if (!it) {
        return DELTA_ITEM_NOT_FOUND;
    }
    ...
    if (res + 2 <= it->nbytes && it->index->refcount == 2) {
        // item_stats_sizes_remove_nvm(it);
        ITEM_set_cas(it, (settings.use_cas) ? get_cas_id() : 0);
        // item_stats_sizes_add_nvm(it);
        memcpy(ITEM_data(it), buf, res);
        memset(ITEM_data(it) + res, ' ', it->nbytes - res - 2);
        do_item_update_nvm(it);
    } else if (it->index->refcount > 1) {
        item_nvm *new_it;
        uint32_t flags;
        if (settings.inline_ascii_response) {
            flags = (uint32_t) strtoul(ITEM_suffix(it), (char **) NULL, 10);
        ...
        // TODO TODO TODO 2018.10.07
        new_it = do_item_alloc_nvm(ITEM_key(it), it->nkey, flags, it->exptime, res + 2);
        if (new_it == 0) {
            do_item_remove_nvm(it);
            return EOM;
        }
        memcpy(ITEM_data(new_it), buf, res);        
        memcpy(ITEM_data(new_it) + res, "\r\n", 2);     
        item_replace_nvm((item *)it, (item *)new_it, hv);
        ITEM_set_cas(it, (settings.use_cas) ? ITEM_get_cas(new_it) : 0);
        do_item_remove_nvm(new_it);       /*  release our reference */
    } else {
        /*  Should never get here. This means we somehow fetched an unlinked
         * item. TODO: Add a counter? */
        if (settings.verbose) {
    ...
    do_item_remove_nvm(it);         /*  release our reference */
    return OK;
}
...
int main (int argc, char **argv) {
    ...
    settings.hashpower_init = 23;
    assoc_init(settings.hashpower_init);
    settings.hashpower_init = 19;
